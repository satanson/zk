...
namespace org{namespace apache{namespace zookeeper{
    ...
    class Watcher{
        ...
        public:
            ...
            virtual void process(WatchedEvent event){
                LOGi("Events are processed by default watcher");
            }
    };
    class ZooKeeper{
        public:
            void watch_callback(WatchedEvent event){
        ...
        public:
            ...
            ZooKeeper(string const& connStr,int timeout,
                    shared_ptr<Watcher> watcher,int64_t sessionId,
                    string const& passwd,bool readable){
                ...
                zk_handle = zookeeper_init(connStr.c_str(), watch_process, timeout,
                        &zk_clientid, NULL,readable);
                if (!zk_handle){
                    THROW(zerror(errno));
                }
                zk_watcher=watcher;
            ...
            ZooKeeper(string const& connStr, int timeout, shared_ptr<Watcher> watcher,
                    int64_t sessionId, string const& passwd):
                ZooKeeper(connStr,timeout,watcher,sessionId,passwd,false){}

            ZooKeeper(string const& connStr, int timeout, shared_ptr<Watcher> watcher, bool readable):
                ZooKeeper(connStr,timeout,watcher,0,string(16,'0'),false){}

            ZooKeeper(string const& connStr, int timeout,shared_ptr<Watcher> watcher):
                ZooKeeper(connStr,timeout, watcher,false){}
